...
export function createInstr(nCorrect: number = NaN, duration: number[] | typeof NaN = NaN) {
    const instr_line = {
        timeline: [slide_line].concat(loop_trial),
        loop_function: function () {
            var accuracy = jsPsych.data.get().last(7).filter({ screenID: "retrieval", acc: true }).count();
            var mean_RT = jsPsych.data.get().last(10).filter({ screenID: "retrieval" }).select('rt').mean();

            let acc_return = false;
            let RT_return = false;

            if (accuracy >= nCorrect || Number.isNaN(nCorrect)) acc_return = true;
            if ((mean_RT > duration[0] && mean_RT < duration[1]) || Number.isNaN(duration)) RT_return = true;
